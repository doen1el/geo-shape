...
async function build(key) {
	...
	for (const f of simplified) {
		...
		items.push({ p, path: d, name: label.name, nameDe: label.nameDe ?? label.name, answers: dedupAnswers([label.name, ...label.answers]), info: cfg.info?.(shape, p), capital });
	}
	...
	writeCategoryFile(key, cfg, { paths, names, namesDe, answers, info, capitals });
...
function writeCategoryFile(key, cfg, { paths, names, namesDe, answers, info, capitals }) {
